...
func init() {
	...
	statementUC := statement.NewStatementUseCase(database.Pool)
	userUC := user.NewUserUseCase(database.Pool)
	telegramHandler = telegram.NewHandler(expenseUC, investmentUC, revenueUC, summaryUC, statementUC, userUC)
